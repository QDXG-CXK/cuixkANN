...
template <typename T>
void load_dataset(std::unique_ptr<BinDataset<T>> &dataset, std::string name, std::string conf_path)
{ 
  nlohmann::json conf;
  ...
  if (conf.contains(name)){
    base_path = conf.at(name).at("base");
    query_path = conf.at(name).at("query");
    gt_path = conf.at(name).at("groundtruth");
  }
  else{
    throw std::runtime_error("can't parse dataset " + name);
  }

  dataset = std::make_unique<BinDataset<T>>(name, base_path, 0, 0, query_path);
  if(!dataset) { throw std::runtime_error("Fail to create BinDataset."); }
}